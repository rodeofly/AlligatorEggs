 ...
 ###########################################################################################################################################################
  ...
  find_action_pointer = (root) ->
    ...
    while (pointer.length > 0) # TOP-LEFT RULE
      ...
      if (pointer.hasClass "priorite")  # On tombe sur un croco blanc
        break if (pointer.children(":not(.svg-container)").length is 1) #il n'y en a qu'un -> ce croco blanc ne sert à rien
  ...
  render_viewport = () ->
    init : (pointer) ->
      ...
      if particles.length > 0
        particles.each ->
          ...
          target = particle.prev(":not(.svg-container)")
          if target.length > 0
            sys.addEdge("#{particle.attr('id')}", target.attr("id"))
          else
            parent = particle.parent()
            sys.addEdge("#{particle.attr('id')}", parent.attr("id")) if parent.attr("id") isnt "root"
      sys.renderer = Renderer("#viewport") 
      
    step2 : (pointer) ->
      local_debug = true
      console.log("step 2") if local_debug 
      firstchild  = pointer.children(":not(.svg-container)").first().attr("id")
      target = pointer.prev(":not(.svg-container)")
      if target.length > 0
        sys.addEdge( target.attr("id"), firstchild)
      parent = pointer.parent()
      sys.addEdge(parent.attr("id"), firstchild) if parent.attr("id") isnt "root"
      sys.pruneNode( pointer.attr("id") )

    step3 : (element) ->
      local_debug = true
      console.log("step 3") if local_debug 
    ...
    step4 : (pointer, application) ->
      local_debug = true
      console.log("step 4") if local_debug        
      application.find(".dropped").addBack().each -> sys.pruneNode( $(this).attr("id") )
      heritier  = pointer.children(":not(.svg-container)").attr("id")
      console.log("heritier du defunt #{pointer.attr('id')} : #{heritier}")

      target = pointer.prev(":not(.svg-container)")
      if target.length > 0
        sys.addEdge( heritier, target.attr("id") )
        console.log(target.attr("id") + " p-> " + heritier) if local_debug
      source = pointer.next(":not(.svg-container)").next(":not(.svg-container)")
      if source.length > 0
        sys.addEdge(source.attr("id"), heritier)
        console.log(source.attr("id") + " n-> " + heritier) if local_debug
      parent = pointer.parent()
      sys.addEdge(parent.attr("id"), heritier) if parent.attr("id") isnt "root"
    ...
    step5 : (egg, application) ->
      local_debug = true
      console.log("step 5") if local_debug 

      if application.length
        application_id = application.attr("id")
        application.find(".dropped").addBack().each -> transform_div_to_node $(this)
        application.find(".dropped").addBack().each ->
          element = $(this)
          switch element.attr("id")
            when application_id
              console.log("application root:{element.attr('id')}") if local_debug
              heritier = application_id
              s = egg
            else
              console.log("application element:#{element.attr('id')}") if local_debug
              heritier = element.attr("id")
              s = element
          
          target = s.prev(":not(.svg-container)")
          if target.length > 0
            console.log(heritier + " prev-> " + target.attr("id")) if local_debug
            sys.addEdge( heritier, target.attr("id") )
            
          source = s.next(":not(.svg-container)")
          if source.length > 0
            console.log(source.attr("id") + " next-> " + heritier) if local_debug
            sys.addEdge(source.attr("id"), heritier)
      ...
      #On efface l'oeuf 
      console.log("step 5 is over !") if local_debug  
  ...
  go_one_step = (root, button) ->
    ...
    step4.done (pointer, app) ->      
      ...
      eggs.each ->
        egg = $(this)
        app.find(".dropped").addBack().each -> 
          $(this).attr "id", id += 1
        render_viewport().step5(egg, app)
      n = eggs.length
      if n>0
        def_clone = $.Deferred()
        def_egg = $.Deferred()
        eggs.each (index, element) ->
          $(this).animate { opacity: 0} , delta, ->
            ...
            if index is n-1
              def_egg.resolve()
          $(this).after app.clone().css(opacity: 0).animate {opacity: 1}, delta, ->
            if index is n-1          
              def_clone.resolve()

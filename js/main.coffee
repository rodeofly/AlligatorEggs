...
$ ->
  ...
  do initialize_html = ->
    ...
    html= ""
    html += "<button id='#{key}' class='panel-button' data-type='fonction' data-lambda='#{value}'>#{key}</button>" for key,value of FUNCTION
  ...
  $( "#go" ).on "click", ->
    local_debug = false
    delta = if speed then 0 else 4000
    ahead_vars = []
    ...
    decapsule = (pointer) ->
      ...
      pointer[0] = $(pointer).find( "svg" ).first().remove()[0]
    while stay
      ...
      if (pointer.hasClass "lambda") and (not pointer.hasClass "priorite")
        ...
        switch pointer.next().length
          when 0
            ...
            if (pointer.next().hasClass( "priorite" )) and (pointer.next().find(".lambda:not(.priorite)").next().length > 0)
              pointer = pointer.next()
              continue
            else
              #c'est bon action !
    ...
    if (pointer.hasClass "lambda") and (pointer.attr("data-color") isnt "white") and (pointer.next().length > 0)
      ...
      do color_rule_check = (pointer, application) ->
        ...
        closestCommonAncestor = (a, b) -> a.parents().has(b).first()
        get_vars = (tree) ->
          palette = []
          tree.find( "[data-variable]" ).andSelf().filter("[data-variable]").not(".lambda.priorite").each -> 
            if $( this ).data("variable") not in ahead_vars
              palette.push $( this ).data("variable")
          palette.unique()
        find_tree = (element, racine_id) ->
          i = element.attr("id")
          tree = $("##{racine_id}").find(".dropped").has( "##{i}" )
        common_ancestor = closestCommonAncestor pointer, application
        i = "##{pointer.attr('id')}"
        j = "##{application.attr('id')}"
        #alert "ahead_vars : " + ahead_vars
        common_ancestor = common_ancestor.clone()
        common_ancestor.find(i).remove()
        common_ancestor.find(j).remove()
        #alert "ancestor : #{common_ancestor.attr("id")} to_children : #{common_ancestor.find( "[data-variable]" ).andSelf().filter("[data-variable]").not(".lambda.priorite").length}"
        if common_ancestor.attr("id") isnt "root"
          common_ancestor.find( "[data-variable]" ).andSelf().filter("[data-variable]").not(".lambda.priorite").each -> 
            ahead_vars.push $(this).attr "data-variable"
        ...
        #Et on va lister toutes les couleurs de l'application
        ...
        if intersection.length
          ...
          palette = (item for item in ALPHABET when item not in used_vars)
          palette = palette[0..intersection.length-1]   
          #alert "app vars: #{application_vars} | func_vars : #{function_vars} | intersect : #{intersection} | palette : #{palette}"
